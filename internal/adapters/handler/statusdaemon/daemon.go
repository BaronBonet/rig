...
func (d *Daemon) Serve(ctx context.Context) error {
	...
	if d.hookListenAddr == "" && d.hookListener == nil {
		return fmt.Errorf("status daemon hook listen addr not configured")
	}
	if d.service == nil {
		return fmt.Errorf("status daemon task service not configured")
	}

	hookListener := d.hookListener
	var err error
	if hookListener == nil {
		hookListener, err = net.Listen("tcp", d.hookListenAddr)
		if err != nil {
			return fmt.Errorf("listen for status daemon hook ingestion: %w", err)
		}
	}
	...
	hookMux.Handle("/codex-hook", d.codexHookHandler())
	hookMux.Handle("/hook", d.codexHookHandler())
