...
func (b *Bootstrapper) BootstrapTaskWorkspace(_ context.Context, task *core.Task) error {
	...
	if err := os.WriteFile(filepath.Join(task.WorktreePath, ".codex", "hooks.json"), []byte(renderHooksJSON()), 0o644); err != nil {
		return err
	}

	if err := os.WriteFile(
		filepath.Join(hooksDir, "forward-to-collector.sh"),
		[]byte(renderForwarderScript(b.sqlitePath, b.collectorURL, b.agentExec, b.sourceRoot)),
		0o755,
	); err != nil {
		return err
	}
