...
func TestNewRepository_SeedsMigrationStateForExistingLatestDatabase(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state.db")
...
create table task_hook_sessions (
...
create table task_observer_summaries (
  task_id text primary key,
...
func TestNewRepository_NormalizesPartiallyPopulatedLegacyMigrationState(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state.db")
...
create index idx_task_hook_events_task_occurred_at on task_hook_events(task_id, occurred_at desc, id desc);
create index idx_task_hook_sessions_session_id on task_hook_sessions(session_id);
...
`)
	...
	versions := migrationVersions(t, repo.db)
	require.NotContains(t, versions, "000001_sqlc_bootstrap")
	require.Contains(t, versions, "000001_initial_tasks_and_events")
	require.Contains(t, versions, "000002_add_task_metadata_columns")
	require.Contains(t, versions, "000003_add_hook_observability_tables")
	require.Len(t, versions, 3)
}

func TestSeedLegacyMigrationState_DoesNotCertifyIncompleteNamedSchemas(t *testing.T) {
...
func TestNewRepository_FailsForHookSessionTableWithCompositeTaskIDPrimaryKey(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state.db")
...
`)
	...
	err = repo.IsAvailable(context.Background())
	require.Error(t, err)
	require.Contains(t, err.Error(), "incomplete managed schema for task_hook_sessions.task_id uniqueness")
}

func TestNewRepository_FailsForObserverSummaryTableWithPartialTaskIDUniqueIndex(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state.db")
...
create index idx_task_hook_events_task_occurred_at on task_hook_events(task_id, occurred_at desc, id desc);
create index idx_task_hook_sessions_session_id on task_hook_sessions(session_id);
create unique index idx_task_observer_summaries_task_id_partial on task_observer_summaries(task_id)
where updated_at <> '';
`)
	require.NoError(t, err)
	require.NoError(t, db.Close())
	...
	err = repo.IsAvailable(context.Background())
	require.Error(t, err)
	require.Contains(t, err.Error(), "incomplete managed schema for task_observer_summaries.task_id uniqueness")
}

func TestNewRepository_CompletesPartialTaskMetadataSchema(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "state.db")
...
`)
	...
	taskColumns := tableColumns(t, repo.db, "tasks")
	for _, column := range []string{
		"repo_name",
		"agent_window_name",
		"editor_window_name",
		...
		require.Contains(t, taskColumns, column)
	}
...
func migrationVersions(t *testing.T, db *sql.DB) map[string]struct{} {
	t.Helper()
